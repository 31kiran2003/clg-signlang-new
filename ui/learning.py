...
class LearningSection(QWidget):
    ...
    def update_card_layout(self):
        ...
        # Adjust column calculation for smaller cards
        columns = max(2, min(3, window_width // 400))
    ...
    def create_category_card(self, title, image_path, description, color):
        # Define a common width for both card and button
        common_width = 400
        
        card = QFrame()
        # Significantly reduce card size from 450x550 to 400x450
        card.setFixedSize(common_width, 450)
        ...
        # Reduce button size but keep width same as card
        learn_btn.setFixedSize(common_width, 45)
        learn_btn.setIconSize(QSize(20, 20))
        button_color = ""
    ...
    def load_local_image(self, image_label, image_path):
        if os.path.exists(image_path):
            ...
            # Reduce image size to match card width and maintain aspect ratio
